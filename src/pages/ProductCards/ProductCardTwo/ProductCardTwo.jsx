...
const ProductCardTwo = ({ product, index }) => {
    // console.log(product);
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 flex space-x-2">
        {product?.productType === "New Arrivals" && (
          <span className="bg-red-600 text-white w-auto px-2 py-1 text-[15px] font-primary font-medium text-center">
            New
          </span>
        )}
        {product?.productType === "Most Popular" && (
          <span className="bg-emerald-500 text-white w-auto px-2 py-1 text-[15px] font-primary font-medium text-center">
            Popular
          </span>
        )}
        {product?.discountPercentage > 0 && (
          <span className="bg-black text-white w-auto px-2 py-1 text-[15px] font-primary font-medium text-center">
      ...
      <Link to={`/product-details/${product?.productUrl}`}>
        <p className="text-[15px] font-primary font-medium text-center">{product?.title}</p>
      </Link>
      {product?.styleNumber && (
