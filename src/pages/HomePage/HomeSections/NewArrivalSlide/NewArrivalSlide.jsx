...
const NewArrivalSlide = () => {
  ...
  const {
    isLoading: isNewArrivalsProductsLoading,
    data: newArrivalsProducts = [],
  } = useQuery({
    queryKey: ["newArrivalsProducts", language],
    queryFn: async () => {
      const res = await axiosPublic.get("/products", {
        params: { language, productType: "New Arrivals", visibility: "Public" },
      });
  ...
  if (isNewArrivalsProductsLoading) {
    return (
      <div>
        <Loader></Loader>
      </div>
    ...
    <div className="bg-light dark:bg-dark text-light dark:text-dark">
      <div className="w-full h-full max-w-[100%] sm:max-w-[640px] md:max-w-[768px] lg:max-w-[1024px] xl:max-w-[1280px] 2xl:max-w-[1536px] mx-auto px-2.5 md:px-5 lg:px-0 pb-5 lg:pb-10">
        ...
        <div className="tittle flex justify-center items-center gap-5 mb-8">
          ...
          />
          <BlurText
            text={t("newArrivalSlideHome.title")}
            delay={150}
            animateBy="words"
            direction="top"
        ...
        <div className="relative w-full h-full">
          ...
          <div
            className={`${
              newArrivalsProducts?.length > 2 ? "block" : "hidden"
            } ${newArrivalsProducts?.length > 4 ? "lg:block" : "lg:hidden"} ${
              newArrivalsProducts?.length > 4 ? "2xl:block" : "2xl:hidden"
            }`}
          >
            <button
