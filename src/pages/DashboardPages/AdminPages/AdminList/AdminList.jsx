...
const AdminList = () => {
  ...
  const {
    ...
    data: allAdminResponse = {}, // Default to an empty object
    refetch,
    isPreviousData,
  } = useQuery({
    queryKey: ["allAdmin", currentPage],
    queryFn: async () => {
      const res = await axiosSecure.get("/allUser", {
        params: { currentPage, limitPerPage, searchQuery, role: "Admin" },
      });
      return res.data;
    },
  });
  ...
  const { data: allUser = [], totalUsers, hasMore } = allAdminResponse;
  ...
  return (
    <div className="w-full h-full relative bg-gray-100 dark:bg-gray-800 rounded">
      <Helmet>
        <title>FreeFeel | Dashboard - Admin List</title>
      ...
      <div className="px-4 py-3 border-b border-dashed border-gray-300 dark:border-gray-600 flex justify-between items-center">
        <div className="w-full lg:w-auto flex justify-between items-center">
          <h3 className="w-full font-primary text-lg font-medium text-start">Total Admin -  {totalUsers}</h3>
