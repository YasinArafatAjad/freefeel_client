...
const Terms = () => {
  ...
  return (
    <div className="w-full h-full relative bg-gray-100 dark:bg-gray-800 rounded">
      <Helmet>
        <title>FreeFeel | Dashboard - Terms Of Service</title>
        <meta name="description" content="Explore terms of service on the FreeFeel Dashboard." />
      </Helmet>
      {isUpdateInfo ? (
        <div>
        ...
        <>
          <div className="px-4 py-3 border-b border-dashed border-gray-300 dark:border-gray-600 flex justify-between items-center">
            <h3 className="font-primary text-lg font-medium">Terms Of Service</h3>

            <div className="flex items-center gap-2.5">
              <button
                onClick={() => setIsUpdateInfo("Terms Of Service")}
          ...
          <div className="px-2.5 py-1.5 lg:px-5 lg:py-2.5 space-y-5">
            <div className="bg-white dark:bg-black rounded-lg shadow p-5 flex flex-col items-start">
              <p className="text-[15px] text-light dark:text-dark">{renderDescription(websiteInfo.termsOfServiceEng)}</p>
            </div>

            <div className="bg-white dark:bg-black rounded-lg shadow p-5 flex flex-col items-start">
              <p className="text-[15px] text-light dark:text-dark">{renderDescription(websiteInfo.termsOfServiceBan)}</p>
            </div>
          </div>
        </>
      )}
