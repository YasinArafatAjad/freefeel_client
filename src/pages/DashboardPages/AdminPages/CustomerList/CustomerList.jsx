...
const CustomerList = () => {
  ...
  // Fetch all customer
  const {
    isLoading: isAllCustomerLoading,
    data: allCustomerResponse = {}, // Default to an empty object
    refetch,
    isPreviousData,
  } = useQuery({
    queryKey: ["allCustomer", currentPage],
    queryFn: async () => {
      const res = await axiosSecure.get("/allUser", {
        params: { currentPage, limitPerPage, searchQuery, role: "Customer" },
      });
      return res.data;
    },
  });
  ...
  const { data: allUser = [], totalUsers, hasMore } = allCustomerResponse;
  ...
  return (
    <div className="w-full h-full relative bg-gray-100 dark:bg-gray-800 rounded">
      <Helmet>
        <title>FreeFeel | Dashboard - Customer List</title>
        <meta name="description" content="Explore all users on the FreeFeel Dashboard." />
      </Helmet>
      ...
      <div className="px-4 py-3 border-b border-dashed border-gray-300 dark:border-gray-600 flex justify-between items-center">
        <div className="w-full lg:w-auto flex justify-between items-center">
          <h3 className="w-full font-primary text-lg font-medium text-start">Total Customer - {totalUsers}</h3>
                ...
                <tr key={user?._id} className="border-b border-gray-300 dark:border-gray-600">
                  ...
                  <td className="text-[15px] font-primary font-medium text-light dark:text-dark text-center">
                    <div className="flex justify-center items-center gap-2.5">
                      <Link to={`/dashboard/view-user/${user?._id}`} title="View">
                        <IoEyeOutline className="text-light dark:text-dark" size={22}></IoEyeOutline>
                      </Link>
                      {/* <Link to={`/dashboard/update-order/${user?._id}`} title="Update">
