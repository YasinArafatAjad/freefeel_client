...
const UpdateProduct = () => {
  ...
  // Fetch a specific product by product url
  const {
    ...
    queryKey: ["id", id],
    queryFn: async () => {
      const res = await axiosPublic.get(`/getProductById/${id}`);
  ...
  //   set default value to from filed
  useEffect(() => {
    if (product) {
      const currentValues = watch();
      const isDifferent =
        currentValues.titleEng !== (product?.titleEng || "") ||
        currentValues.titleBan !== (product?.titleBan || "") ||
        currentValues.category !== (product?.category || "");

      if (isDifferent) {
        reset({
          titleEng: product?.titleEng || "",
          titleBan: product?.titleBan || "",
          category: product?.category || "",
          productUrl: product?.productUrl || "",
          productType: product?.productType || "",
          regularPrice: product?.regularPrice || "",
          discountPercentage: product?.discountPercentage || "",
          engColors: product?.engColors ? product.engColors : [],
          quantity: product?.quantity || "",
          stockStatus: product?.stockStatus || "",
          styleNumber: product?.styleNumber || "",
          visibility: product?.visibility || "",
          deliveryStatus: product?.deliveryStatus || "",
          banColors: product?.banColors ? product.banColors : [],
          engSizes: product?.engSizes ? product.engSizes : [],
          banSizes: product?.banSizes ? product.banSizes : [],
          descriptionEng: product?.descriptionEng || "",
          descriptionBan: product?.descriptionBan || "",
          detailsEng: product?.detailsEng || "",
          detailsBan: product?.detailsBan || "",
          sizeGuidelinesEng: product?.sizeGuidelinesEng || "",
          sizeGuidelinesBan: product?.sizeGuidelinesBan || "",
          metaTitle: product?.metaTitle || "",
          metaFocusKeywords: product?.metaFocusKeywords || "",
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const res = await axiosPublic.put(`/updateProduct/${id}`, formData);
  ...
  return (
    <div className="w-full h-full relative bg-gray-100 dark:bg-gray-800 rounded">
      <div className="px-2.5 py-1.5 lg:px-5 lg:py-2.5 border-b border-dashed border-gray-300 dark:border-gray-600 flex justify-between items-center">
        <h3 className="font-primary text-lg font-medium">Update Product</h3>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-12 gap-5">
          ...
          <div className="relative w-full h-full space-y-2 col-span-12 md:col-span-6 lg:col-span-4 2xl:col-span-3">
            ...
            <div className="relative flex items-center gap-2">
              <input
                type="checkbox"
                checked={watch("deliveryStatus") === "Free Delivery"} // Explicitly control checked state
                onChange={(e) =>
                  e.target.checked ? setValue("deliveryStatus", "Free Delivery") : setValue("deliveryStatus", "")
                }
                ...
                <div className="grid grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-2.5">
                  ...
                  <div className="w-full h-48 flex justify-center items-center relative">
                    {selectedThumbnail ? (
                      ...
                      <div className="w-full h-48 relative">
                        <img src={product?.thumbnail} alt="Selected" className="w-full h-48 object-cover rounded" />
                      </div>
                    ) : (
                  ...
                  {selectedPhotos.length > 0 ? (
                    ...
                    <>
                      {product?.photos.map((photo, index) => (
                        <div key={index} className="relative w-full h-48">
                          <img src={photo} alt="Selected" className="w-full h-48 object-cover rounded" />
                        </div>
                      ))}
                    </>
                  ) : (
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-3 gap-2.5 md:gap-3.5 lg:gap-5">
                    <div className="w-full h-full py-5 border border-dashed border-gray-300 dark:border-gray-600 rounded">
                      ...
                      >
                        ...
                        <input
                          {...register("sizeGuideline", { required: false })}
                          id="sizeGuideline"
                          type="file"
                    ...
                    <div className="w-full h-full flex justify-center items-center relative">
                      {selectedSizeGuideline ? (
                        ...
                        <div className="w-full h-full relative">
                          <img src={product?.sizeGuideline} alt="Selected" className="w-full h-full object-cover rounded" />
                        </div>
                      ) : (
                        <p>Please Select an Image</p>
                      )}
